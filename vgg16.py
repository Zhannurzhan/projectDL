...
optimizer = optim.Adam([
    {'params': m.features[17:24].parameters(), 'lr': 1e-6},
    {'params': m.features[24:].parameters(),   'lr': 1e-5},
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(),
                   os.path.join(MODEL_DIR, "VGG16_best.pth"))
...
np.save(os.path.join(RESULTS_DIR, "VGG16_cm.npy"), cm)

model_mb = os.path.getsize(os.path.join(MODEL_DIR, "VGG16_best.pth")) / 1e6
...
print(f"\n  [VGG16] Val={result['best_val_acc']}%  "
      f"F1={result['f1_score']}%  "
      f"Size={result['model_size_mb']}MB  "
      f"Inf={result['inference_ms']}ms")

hist_file = os.path.join(RESULTS_DIR, "VGG16_history.csv")
...
if os.path.exists(cmp_file):
    with open(cmp_file) as f:
        existing = [r for r in csv.DictReader(f) if r['model'] != 'VGG16']
