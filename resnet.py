...
optimizer = optim.Adam([
    {'params': m.layer2.parameters(), 'lr': 1e-7},
    {'params': m.layer3.parameters(), 'lr': 1e-6},
...
np.save(os.path.join(RESULTS_DIR, "ResNet50_cm.npy"), cm)

model_mb = os.path.getsize(os.path.join(MODEL_DIR, "ResNet50_best.pth")) / 1e6
...
print(f"\n  [ResNet50] Val={result['best_val_acc']}%  "
      f"F1={result['f1_score']}%  "
      f"Size={result['model_size_mb']}MB  "
      f"Inf={result['inference_ms']}ms")

hist_file = os.path.join(RESULTS_DIR, "ResNet50_history.csv")
...
if os.path.exists(cmp_file):
    with open(cmp_file) as f:
        existing = [r for r in csv.DictReader(f) if r['model'] != 'ResNet50']
