...
optimizer = optim.Adam([
    {'params': m.inception5a.parameters(), 'lr': 1e-5},
    {'params': m.inception5b.parameters(), 'lr': 1e-5},
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(),
                   os.path.join(MODEL_DIR, "GoogLeNet_best.pth"))
...
np.save(os.path.join(RESULTS_DIR, "GoogLeNet_cm.npy"), cm)

model_mb = os.path.getsize(os.path.join(MODEL_DIR, "GoogLeNet_best.pth")) / 1e6
...
print(f"\n  [GoogLeNet] Val={result['best_val_acc']}%  "
      f"F1={result['f1_score']}%  "
      f"Size={result['model_size_mb']}MB  "
      f"Inf={result['inference_ms']}ms")

hist_file = os.path.join(RESULTS_DIR, "GoogLeNet_history.csv")
...
if os.path.exists(cmp_file):
    with open(cmp_file) as f:
        existing = [r for r in csv.DictReader(f) if r['model'] != 'GoogLeNet']
...
for r in existing:
    print(f"{r['model']:<14} {r['best_val_acc']:>7}%  "
          ...
          f"{r['model_size_mb']:>8}")
print("=" * 55)
print(f"\nNext: python train_efficientnet.py")