...
optimizer = optim.Adam([
    {'params': m.features[6:7].parameters(), 'lr': 1e-6},
    {'params': m.features[7:].parameters(),  'lr': 1e-5},
...
np.save(os.path.join(RESULTS_DIR, "EfficientNet_cm.npy"), cm)

model_mb = os.path.getsize(os.path.join(MODEL_DIR, "EfficientNet_best.pth")) / 1e6
...
print(f"\n  [EfficientNet] Val={result['best_val_acc']}%  "
      f"F1={result['f1_score']}%  "
      f"Size={result['model_size_mb']}MB  "
      f"Inf={result['inference_ms']}ms")

hist_file = os.path.join(RESULTS_DIR, "EfficientNet_history.csv")
...
if os.path.exists(cmp_file):
    with open(cmp_file) as f:
        existing = [r for r in csv.DictReader(f) if r['model'] != 'EfficientNet']
...
print("=" * 55)
print(f"\nAll 5 models done!")
print(f"Next: python evaluate_test.py")